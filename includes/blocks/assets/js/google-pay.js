...
let google_pay_transaction_id = null;
let google_pay_transaction_status = null;
...
const GooglePayBtn = ({
    ...
    const onGooglePaymentButtonClicked = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                merchant_reference: initialDataGooglePay.order_id + " - " + initialDataGooglePay.shop_name,
            ...
            shippingAddress.first_name = paymentResponse.shippingAddress?.name ?? null;
            shippingAddress.last_name = paymentResponse.shippingAddress?.name ?? null;
            shippingAddress.address_1 = paymentResponse.shippingAddress?.address1 ?? null;
            shippingAddress.address_2 = paymentResponse.shippingAddress?.address2 ?? null;
            shippingAddress.city = paymentResponse.shippingAddress?.locality ?? null;
            shippingAddress.state = paymentResponse.shippingAddress?.administrativeArea ?? null;
            shippingAddress.country = paymentResponse.shippingAddress?.countryCode ?? "DE";
            shippingAddress.postcode = paymentResponse.shippingAddress?.postalCode ?? null;
            shippingAddress.email = paymentResponse.email ?? null;
            shippingAddress.phone = paymentResponse.shippingAddress?.phoneNumber ?? null;
            ...
            if (response.ok) {
                ...
                if (data.error_code === 0) {
                    google_pay_transaction_id = data.transaction_id;
                    google_pay_transaction_status = data.status;
    ...
    React.useEffect(() => {
        ...
        const unsubscribe = onPaymentProcessing(async () => {
            // Conditional is used to not pass metadata in other checkout payment methods (i.e. Credit Card, Apple Pay)
            if (google_pay_transaction_id && google_pay_transaction_status) {
                return {
                    type: emitResponse.responseTypes.SUCCESS,
                    meta: {
                        paymentMethodData: {
                            google_pay_transaction_id: google_pay_transaction_id ?? "",
                            google_pay_transaction_status: google_pay_transaction_status ?? "",
                            google_pay_order_id: initialDataGooglePay.order_id,
                        },
                    },
                };
            }
